...
import cn.zcbdqn.commoninventory.utils.zkprint.StatusBox;
 ...
 */
public class SettingPrintActivity extends Activity implements OnItemClickListener, OnClickListener {
	/**
	 * 蓝牙打印机列表显示
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		backBtn=(ImageView) findViewById(R.id.setting_print_back);
	...
	public boolean getBluetoothDevice() {
        ...
        for (BluetoothDevice device : pairedDevices) {
        	...
        	if(btAddress.equals(defaultBluePrintAddress)){
        		map.put("default", getResources().getString(R.string.default_print));
        	}else{
        		map.put("default", "");
        	}
        	...
        	map.put("printName", getResources().getString(R.string.print_name)+device.getName());
        	map.put("printAddress", getResources().getString(R.string.print_address)+device.getAddress());
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		//保存至文件中
		sp.edit().putString("defaultBluePrintAddress", defaultBluePrintAddress).apply();
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.setting_print_back:
			//点击返回,跳转至主设置界面
			Intent intent=new Intent();
			intent.putExtra("resultCode", 1);
			//Toast.makeText(this, "click back", Toast.LENGTH_LONG).show();
			setResult(MainSettingActivity.SETTING_PRINT_CODE,intent);
			finish();
			break;
